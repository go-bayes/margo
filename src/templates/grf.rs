...
pub fn get_template_files(project_name: &str) -> Vec<(String, String)> {
    ...
    let mut files = vec![
        ...
        ("src/00-preflight.R".to_string(), script_00_preflight()),
...
pub fn get_template_files_with_config(
    ...
    let mut files = vec![
        (
            "study.toml".to_string(),
            study_toml_configured(
                project_name,
                pull_data,
                push_mods,
                exposure,
...
# source Arrow object name without extension, for example "nzavs_arrow"
source_arrow_name = "nzavs_arrow"
...
| Script | Purpose |
|--------|---------|
| src/00-preflight.R | shared package, path, and object checks |
| src/00-setup.R | {setup_label} |
...
fn script_00_preflight() -> String {
    r####"# 00-preflight.R
# shared preflight checks for generated GRF workflow
# generated by margo

load_required_packages <- function(packages, github = character()) {
  packages <- unique(packages)
  missing <- packages[!vapply(packages, requireNamespace, logical(1), quietly = TRUE)]

  if (length(missing) > 0) {
    github_missing <- intersect(missing, names(github))
    cran_missing <- setdiff(missing, github_missing)
    ...
    if (length(github_missing) > 0) {
      if (!requireNamespace("devtools", quietly = TRUE)) {
        install_lines <- c(install_lines, 'install.packages("devtools")')
      }
      install_lines <- c(
        install_lines,
        sprintf('devtools::install_github("%s")', unname(github[github_missing]))
      )
    }
...
read_study_config <- function(path = here::here("study.toml")) {
  if (!file.exists(path)) {
    stop(sprintf("config not found: %s", path), call. = FALSE)
  }
  RcppTOML::parseTOML(path)
}

resolve_output_path <- function(cfg) {
  fs::path_expand(require_cfg(cfg$paths$push_mods, "set paths.push_mods in study.toml"))
}

check_output_path <- function(path) {
  if (!dir.exists(path)) {
    dir.create(path, recursive = TRUE)
    cli::cli_alert_info(sprintf("created output directory: %s", path))
  }

  sentinel <- file.path(path, ".margo-write-test")
  ok <- tryCatch({
    ...
    stop(
      sprintf("cannot write to paths.push_mods: %s\n%s", path, conditionMessage(e)),
      call. = FALSE
    )
  })
...
check_required_objects <- function(names, path) {
  missing <- names[!file.exists(file.path(path, paste0(names, ".rds")))]
  if (length(missing) > 0) {
    stop(
      sprintf(
        "missing required saved object(s) in %s: %s",
        path,
...
cran_packages <- c(
  "tidyverse", "here", "fs", "arrow", "data.table", "fastDummies",
  "naniar", "skimr", "grf", "ranger", "kableExtra", "ggplot2",
  "doParallel", "janitor", "stringr", "patchwork", "table1", "cli",
  "glue", "RcppTOML", "rlang", "purrr", "future", "furrr",
  "policytree", "devtools"
)
...
# libraries ---------------------------------------------------------------
source(file.path("src", "00-preflight.R"))
load_required_packages(
  c(
    "margot", "tidyverse", "here", "fs", "arrow", "data.table",
    "fastDummies", "naniar", "skimr", "grf", "kableExtra",
    "ggplot2", "doParallel", "janitor", "stringr", "patchwork",
    "table1", "cli", "RcppTOML", "rlang", "purrr"
  ),
  github = c(margot = "go-bayes/margot")
)
...
source_arrow_name <- cfg$paths$source_arrow_name %||% "nzavs_arrow"
push_mods <- resolve_output_path(cfg)
check_output_path(push_mods)
...
# load data ---------------------------------------------------------------
# source data are expected as an Arrow object, for example nzavs_arrow.parquet
dat <- margot::here_read_arrow(source_arrow_name, pull_path)
...
margot::here_save(n_total_pretty, "n_total", push_mods)
...
margot::here_save(var_labels_exposure, "var_labels_exposure", push_mods)
margot::here_save(baseline_vars, "baseline_vars", push_mods)
margot::here_save(exposure_var, "exposure_var", push_mods)
margot::here_save(name_exposure_binary, "exposure_var_binary", push_mods)
margot::here_save(outcome_vars, "outcome_vars", push_mods)
margot::here_save(baseline_wave, "baseline_wave", push_mods)
margot::here_save(exposure_waves, "exposure_waves", push_mods)
margot::here_save(outcome_wave, "outcome_wave", push_mods)
margot::here_save(ordinal_columns, "ordinal_columns", push_mods)
...
margot::here_save(lower_cut, "lower_cut", push_mods)
margot::here_save(upper_cut, "upper_cut", push_mods)
margot::here_save(threshold, "threshold", push_mods)
margot::here_save(inverse_threshold, "inverse_threshold", push_mods)
margot::here_save(scale_range, "scale_range", push_mods)
margot::here_save(cut_points, "cut_points", push_mods)
...
missing_var_plot <- naniar::gg_miss_var(dat_long_final)
print(missing_var_plot)
margot::here_save(missing_var_plot, "missing_var_plot", push_mods)
...
margot::here_save(t0_sample_weights, "t0_sample_weights", push_mods)
...
load_required_packages(
  c(
    "margot", "tidyverse", "here", "fs", "data.table",
    "fastDummies", "naniar", "skimr", "grf", "kableExtra",
    "ggplot2", "doParallel", "janitor", "stringr", "patchwork",
    "table1", "cli", "glue", "RcppTOML", "rlang", "purrr"
  ),
  github = c(margot = "go-bayes/margot")
)
...
check_required_objects(
  c(
    "dat_long_final", "t0_sample_weights", "name_exposure", "baseline_vars",
    "exposure_var", "outcome_vars", "ordinal_columns"
  ),
  push_mods
)
...
t0_sample_weights <- margot::here_read("t0_sample_weights", push_mods)
name_exposure <- margot::here_read("name_exposure", push_mods)
baseline_vars <- margot::here_read("baseline_vars", push_mods)
exposure_var <- margot::here_read("exposure_var", push_mods)
outcome_vars <- margot::here_read("outcome_vars", push_mods)
ordinal_columns <- margot::here_read("ordinal_columns", push_mods)
...
margot::here_save(df_wide, "df_wide", push_mods)
...
margot::here_save(df_wide_encoded, "df_wide_encoded", push_mods)

cli::cli_h1("encoded wide data")

# two-stage IPCW weights --------------------------------------------------
df <- margot::here_read("df_wide_encoded", push_mods)
...
margot::here_save(df_grf, "df_grf", push_mods)

n_observed_grf <- nrow(df_grf)
margot::here_save(n_observed_grf, "n_observed_grf", push_mods)
...
load_required_packages(
  c(
    "margot", "tidyverse", "here", "fs", "data.table", "grf",
    "ranger", "doParallel", "kableExtra", "ggplot2", "rlang",
    "purrr", "patchwork", "janitor", "glue", "cli", "future",
    "furrr", "RcppTOML"
  ),
  github = c(margot = "go-bayes/margot")
)
...
check_output_path(push_mods)
check_required_objects(
  c("df_wide", "df_grf", "name_exposure", "outcome_vars", "E"),
  push_mods
)
...
title_binary <- title
margot::here_save(title_binary, "title_binary", push_mods)
margot::here_save(title, "title", push_mods)
filename_prefix <- cfg$titles$filename_prefix %||% "grf_results"

label_mapping_all <- labels_all$outcome %||% list()
margot::here_save(label_mapping_all, "label_mapping_all", push_mods)
...
margot::here_save(models_binary, "models_binary", push_mods)
...
load_required_packages(
  c(
    "margot", "tidyverse", "here", "fs", "grf", "kableExtra",
    "ggplot2", "patchwork", "cli", "glue", "RcppTOML"
  ),
  github = c(margot = "go-bayes/margot")
)
...
push_mods <- resolve_output_path(cfg)
check_output_path(push_mods)
check_required_objects(c("models_binary", "label_mapping_all"), push_mods)

# load model --------------------------------------------------------------
models_binary <- margot::here_read("models_binary", push_mods)
label_mapping_all <- margot::here_read("label_mapping_all", push_mods)
...
load_required_packages(
  c(
    "margot", "tidyverse", "here", "fs", "grf", "policytree",
    "kableExtra", "ggplot2", "patchwork", "cli", "glue", "RcppTOML"
  ),
  github = c(margot = "go-bayes/margot")
)
...
source(file.path("src", "00-preflight.R"))
load_required_packages(
  c("margot", "tidyverse", "here", "fs", "kableExtra", "cli", "RcppTOML"),
  github = c(margot = "go-bayes/margot")
)
...
push_mods <- resolve_output_path(cfg)
check_output_path(push_mods)
check_required_objects(c("df_grf", "name_exposure"), push_mods)
...
source(file.path("src", "00-preflight.R"))
load_required_packages(
  c("margot", "tidyverse", "here", "fs", "kableExtra", "table1", "cli", "RcppTOML"),
  github = c(margot = "go-bayes/margot")
)
...
push_mods <- resolve_output_path(cfg)
check_output_path(push_mods)
check_required_objects(c("df_wide", "baseline_vars", "outcome_vars"), push_mods)
...
source(file.path("src", "00-preflight.R"))
load_required_packages(
  c("margot", "tidyverse", "here", "fs", "ggplot2", "patchwork", "cli", "RcppTOML"),
  github = c(margot = "go-bayes/margot")
)
...
push_mods <- resolve_output_path(cfg)
check_output_path(push_mods)
check_required_objects(c("dat_long_1", "name_exposure", "label_mapping_all"), push_mods)
